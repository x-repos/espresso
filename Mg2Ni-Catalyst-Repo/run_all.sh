...
ALL_INPUTS=(mg2ni_Nb mg2nih4_Nb mg2ni_NbFe mg2nih4_NbFe mgo nb2o5 mg2ni_NbFeMg mg2nih4_NbFeMg)
INPUTS=("$@")
if [ ${#INPUTS[@]} -eq 0 ]; then
    INPUTS=("${ALL_INPUTS[@]}")
fi
...
for name in "${INPUTS[@]}"; do
    if [ ! -f "$name.in" ]; then
        echo "SKIP $name: $name.in not found"
        continue
    fi
    echo ""
    ...
    pw.x -in "$name.in" > "$name.out" 2> "logs/$name.err" || {
    ...
    t1=$(date +%s)
    elapsed=$((t1 - t0))
    energy=$(grep '^!' "$name.out" | tail -1 | awk '{print $5}')
